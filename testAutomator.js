...
workbook.xlsx.readFile('./Examples.xlsx')   //Opening Examples.xlsx file to read experiment conditions
    .then(function() {
        ...
        worksheet.eachRow({ includeEmpty: false }, function(row, rowNumber) {
            ...
            require('child_process').execSync(`node ${server_file} --episodeId ` + row.values[2]+ ' --frequency ' + JSON.stringify(row.values[8]) + ' --profile ' + JSON.stringify(row.values[5])  + ' --targetDevice ' + targetDevice + ' --name ' + exp_name + ' --samsung ' + samsung_ip + ' --duration ' + row.values[6], {stdio:[0,1,2]});
